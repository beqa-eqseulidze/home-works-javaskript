...
import type { AuthMode, HttpMethod } from './types'; // შემოვიტანეთ HttpMethod-იც
import type { IAuthPayload, IAuthResponse } from './interface'; 
...
function handleModalOpen(mode: AuthMode) {
  ...
  form.addEventListener('submit', async (e: Event) => {
    ...
    // განვსაზღვროთ მეთოდი HttpMethod ტიპით
    const requestMethod: HttpMethod = 'POST';

    try {
      const response = await fetch(`${API_URL}${endpoint}`, {
        method: requestMethod, // ვიყენებთ ტიპიზირებულ ცვლადს
